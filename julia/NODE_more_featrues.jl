...
ads1 = df_input[150:end,:]

columns = ["jhu-csse_confirmed_7dav_incidence_prop", "jhu-csse_deaths_7dav_incidence_prop","safegraph_bars_visit_prop", "doctor-visits_smoothed_adj_cli", "google-symptoms_sum_anosmia_ageusia_smoothed_search", "safegraph_restaurants_visit_prop", "hospital-admissions_smoothed_adj_covid19_from_claims", "immune", "quidel_covid_ag_smoothed_pct_positive", "fb-survey_smoothed_wcli"]
columns_short = ["7day_confirmed_prop", "7day_deaths_prop","bars_visit_prop", "doctor-visits", "google_anosmia_ageusia_search", "restaurants_visit_prop", "hospital_admissions", "immune", "pos_pcr", "fb_cli"]
ads1 = ads1[!,columns]
...
plot(transpose(ads[:,200:350]))
plot(transpose(ads[:,350:480]))
...
data_batches = [ ads[:,200:350], ads[:,350:480]]
val_batches = [ ads[:,351:401], ads[:,481:530]]
data_names = ["200_350", "350_480"]
...
for i in 1:1:200
  ...
  if !(current_param in paramsTried)
    ...
    mainTitle = "MI10_avg_$loss_method-$data_name-lr_$lr-epoch_$epoch-dudtsize_$dudt_size-mshoot_lens_$mshoot_len-batch_size_$batch_size"
